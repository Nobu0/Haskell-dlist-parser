...
data Expr
  ...
  | -- | PWildcard
    PConstr String [Pattern] -- 例: Just x, Nothing, etc.
  | PTuple [Pattern] -- 例: (x, y)
  | PList [Pattern] -- 例: [x, y]
  | PCons Pattern Pattern -- 例: x:xs
  deriving (Show, Eq)
...
pTuple = do
  pats <- parens (pattern `sepBy` symbol ",")
  case pats of
    [single] -> return single -- 単一要素はタプルではなくそのまま
...
pList = brackets (PList <$> pattern `sepBy` symbol ",")
