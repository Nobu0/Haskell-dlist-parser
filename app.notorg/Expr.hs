...
import Control.Applicative (Alternative (..), empty, liftA, liftA2, liftA3, optional, some, (<|>))
import Control.Monad (MonadPlus (..), void)
import qualified Data.Set as Set
import Data.Void
import Lexer (Token (..))
import Text.Megaparsec (ErrorItem (..), MonadParsec, Parsec, lookAhead, notFollowedBy, runParser, sepBy1, token, try, (<?>))
...
-- tokenIs test = token test Nothing
tokenIs :: (Token -> Maybe a) -> String -> Parser a
tokenIs test expected = token test (Set.singleton (Label (fromString expected)))
