module Language.TypeSystem.Infer.Expr (inferExpr) where
...
{-}
inferBinOp EAdd e1 e2 = do
  t1 <- fresh
  t2 <- fresh
  t  <- fresh
  (s1, ps1, t1') <- inferExpr e1
  (s2, ps2, t2') <- local (apply s1) $ inferExpr e2
  s3 <- unify (apply s2 t1') t1
  s4 <- unify (apply s3 t2') t2
  let s = s4 `composeSubst` s3 `composeSubst` s2 `composeSubst` s1
      preds = apply s (ps1 ++ ps2 ++ [IsIn "Num" (apply s t)])
  return (s, preds, apply s t)
-}
...
inferLet pat e1 e2 = do
  ...
  let s12 = composeMany [s2, s1]
      ...
      sc = generalize envExt ps1 (applySubst s12 tPat)
...
inferVar x = do
  ...
  case Map.lookup x env of
    Just sigma -> do
      (t, ps) <- instantiate sigma
      return (emptySubst, ps, t)
